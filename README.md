...
A Chrome MV3 extension that translates foreign-language webpages into a user-selected target language **inline**, side-by-side with the original. Click the toolbar icon to translate; click again to hide; click again to show — all without re-fetching.
...
- **Bilingual layout** — translations are inserted right after each source block, with a subtle blue background and accent border so you can read both at once
...
| **Target language** | Defaults to your Chrome UI / operating-system language when it matches one of the built-in top-10 language options, and can be changed any time. |

Click **Save**, then **Test connection** — you should see something like `OK. Sample response: [1] Hola mundo` when Spanish is selected.
...
3. Within ~10-30 seconds, every paragraph gets a translation in your selected target language inserted directly underneath, with a light-blue background and blue left border.
...
- `tests/` — `node --test` unit tests for the pure-logic modules (26 tests)
...
- Target language options are limited to the built-in top-10 languages by total speakers
